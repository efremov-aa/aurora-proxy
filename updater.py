# Aurora v1.3.0 — авто-обновление: сверка версии с GitHub Releases, скачивание,
# sha256-проверка, бэкап текущих файлов, замена модулей.
# ВНИМАНИЕ: официальный репо берётся из env AURORA_UPDATE_REPO
# (например efremov-aa/aurora-proxy); пусто = авто-обновление выключено.
...
_REPO = (os.environ.get("AURORA_UPDATE_REPO") or "").strip()
...
def auto_update(mode):
    """Вызов при старте по env AURORA_AUTO_UPDATE: check|apply (или пусто=выкл)."""
    ...
    def _bg():
        if mode == "check":
            ...
            if r.get("ok") and r.get("update"):
                config.log("update: доступна v%s (панель → Обновить)" % r["latest"])
        else:
            r = apply()
            config.log("update: %s" % (r.get("msg") or r.get("error") or r.get("latest")))
