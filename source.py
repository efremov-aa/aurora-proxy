...
def _xray_bin():
    ...
    cands = [
        ...
        os.path.join(os.path.dirname(sys.executable), "xray"),
        "/usr/local/bin/xray",
    ]
    for c in cands:
...
def keytest(key, timeout=15, port=None):
    ...
    xbin = _xray_bin()
    if not xbin:
        return {"status": "sysfail", "reason": "no-xray-bin", "exit_ip": "-", "ping_ms": ping_ms}
    ...
    try:
        ...
        config.log("keytest: ошибка запуска: %s" % e)
        return {"status": "noip", "reason": "keytest-fail", "exit_ip": "-", "ping_ms": ping_ms}
    finally:
        if proc is not None:
            try:
...
def check_all(bg=True):
    ...
    def run():
        try:
            ...
            stats = {"total": n, "ok": 0, "bad": 0, "noip": 0, "slow": 0, "sysfail": 0, "dead": 0}
            t0 = time.time()
            ...
            dt = int(time.time() - t0)
            msg = "check done: {total:%d, ok:%d, bad:%d, slow:%d, noip:%d, sysfail:%d, dead:%d} за %dс" % (
                stats["total"], stats["ok"], stats["bad"], stats["slow"], stats["noip"],
                stats["sysfail"], stats["dead"], dt)
            _comm_done(msg)
