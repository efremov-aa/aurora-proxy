...
def _fallback_timeout(deadline: Optional[float], default: float) -> float:
    remaining = remaining_timeout(deadline)
    if remaining is None:
        return default
    if remaining <= 0:
        raise TimeoutError('setup deadline exceeded')
...
async def do_fallback(reader, writer, relay_init, label,
    ...
    if use_cf:
        methods.append('cf')
    if proxy_config.tcp_fallback and fallback_dst:
        methods.append('tcp')
    ...
    for method in methods:
        if method == 'cf_worker' and fallback_dst:
            ...
            ok = await _cfproxy_fallback(
                ...
                splitter=splitter, deadline=deadline)
            if ok:
                return True
            ...
            ok = await _tcp_fallback(
                ...
                relay_init, label, ctx, deadline=deadline)
            if ok:
                return True
    return False
...
async def _cfproxy_worker_fallback(reader, writer, relay_init, label,
        ...
        for worker_domain in cf_worker_pool.available_domains(worker_domains):
            ...
            try:
                ws = await RawWebSocket.connect(
                    worker_domain,
                    worker_domain,
                    timeout=_fallback_timeout(deadline, 10.0),
                    path=path,
                )
                break
    ...
    try:
        await await_with_deadline(ws.send(relay_init), deadline)
...
async def _tcp_fallback(reader, writer, dst, port, relay_init, label,
    ...
    try:
        rw.write(relay_init)
        await await_with_deadline(rw.drain(), deadline)
        owned = True
        ...
        log.warning("[%s] TCP fallback session failed: %s",
                    label, repr(exc))
        return False
    finally:
        if not owned:
            await _close_stream_writer(rw)
    ...
    try:
        ...
        for t in tasks:
            try:
                ...
                pass
        await _close_stream_writer(remote_writer)
        await _close_stream_writer(writer)
