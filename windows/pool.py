...
def set_status(uri, **fields):
    n = _norm_uri(uri)
    with _LOCK:
        status_key = _record_key(_STATUS, n, "status")
        st = dict(_STATUS.get(status_key, {}))
        st.update(fields)
        _STATUS[status_key] = st
        _save_status()
...
def prune_status():
    ...
    known_ids = set(crypt.opaque_id(_norm_uri(k.get("uri", "")), "status")
                    for k in _KEYS)
    known_legacy = set(_norm_uri(k.get("uri", "")) for k in _KEYS)
    stale = []
    for key in list(_STATUS):
        if crypt.is_opaque_id(key):
            if key not in known_ids:
                stale.append(key)
        elif key not in known_legacy:
            stale.append(key)
    if stale:
        with _LOCK:
            for key in stale:
...
def dead_add(uri, reason="user_removed"):
    ...
    n = _norm_uri(uri)
    with _LOCK:
        dead_key = _record_key(_DEAD, n, "dead")
        _DEAD[dead_key] = {"reason": reason, "ts": int(time.time())}
        _save_dead()
...
def dead_remove(uri):
    ...
    with _LOCK:
        dead_key = _record_key(_DEAD, n, "dead")
        if dead_key in _DEAD:
            _DEAD.pop(dead_key, None)
            _save_dead()
            return True
    return False
...
def dead_reason(uri):
    n = _norm_uri(uri)
    with _LOCK:
        return _DEAD.get(_record_key(_DEAD, n, "dead"), {}).get("reason")


def blocked(uri):
    n = _norm_uri(uri)
    with _LOCK:
        return _record_key(_DEAD, n, "dead") in _DEAD
