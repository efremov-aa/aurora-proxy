...
from .utils import await_with_deadline, remaining_timeout
...
async def proxy_to_masking_domain(reader, writer, initial_data: bytes,
    ...
    effective_deadline = time.monotonic() + 10.0
    ...
    try:
        up_reader, up_writer = await await_with_deadline(
            asyncio.open_connection(domain, 443), effective_deadline)
    ...
    try:
        ...
        await await_with_deadline(up_writer.drain(), effective_deadline)
    except asyncio.CancelledError:
        raise
    except Exception as exc:
        log.warning("[%s] masking: cannot send client hello: %s",
                    label, repr(exc))
        try:
            up_writer.close()
        except BaseException:
            pass
        transport = getattr(up_writer, 'transport', None)
    ...
    tasks = [
        asyncio.create_task(_relay(reader, up_writer)),
        asyncio.create_task(_relay(up_reader, writer)),
    ]
    try:
        ...
        for stream_writer in (up_writer, writer):
            try:
                stream_writer.close()
                await asyncio.wait_for(
                    stream_writer.wait_closed(), timeout=1.0)
            except BaseException:
                transport = getattr(stream_writer, 'transport', None)
                if transport is not None:
                    transport.abort()
