...
class _WsPool:
    ...
    def _task_done(self, task: asyncio.Task) -> None:
        ...
        try:
            ...
            return
        if exc is not None:
            log.warning("WS pool task failed: %s", type(exc).__name__)
    ...
    def _schedule_refill(self, key, target_ip, domains):
        ...
        self._track(self._refill(key, target_ip, domains))
...
class _CfWorkerPool:
    ...
    def _task_done(self, task: asyncio.Task) -> None:
        ...
        try:
            ...
            return
        if exc is not None:
            log.warning("CF worker pool task failed: %s", type(exc).__name__)
    ...
    async def get(self, dc: int, fallback_dst: str,
        ...
        while bucket:
            ...
            log.debug("CF worker pool hit DC%d via %s (age=%.1fs, left=%d)",
