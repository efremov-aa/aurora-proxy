...
def _vless_ext():
    """Р’РЅРµС€РЅСЏСЏ vless-СЃСЃС‹Р»РєР° РґР»СЏ РїРѕРґРєР»СЋС‡РµРЅРёСЏ Рє РґРѕРјР°С€РЅРµРјСѓ РїСЂРѕРєСЃРё РР—Р’РќР•."""
    ...
    link = "vless://%s@%s:%s?%s#Aurora-Out" % (
        v["uuid"], config.get_public_ip() or v.get("host", ""), v.get("port", 8443), q)
    return {
        "link": link,
        "host": config.get_public_ip() or v.get("host", ""),
