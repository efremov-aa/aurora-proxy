...
1. Скачайте **`Aurora-Setup-1.9.3.exe`** из [Releases](https://github.com/efremov-aa/aurora-proxy/releases)
...
> 🪟 **Известная проблема 1.9.3:** NSSM-переменная `AURORA_DATA_DIR` не доходит до frozen-процесса,
> поэтому данные кладутся в `Aurora\_internal\data` вместо `%LOCALAPPDATA%\Aurora`. Это ломает
> обновление «поверх» и удаление (ошибка 145). Исправление — кандидат в **1.9.4**; до него
> переносите каталог данных вручную. Подробности — в [корневом README](../README.md#-замечания-и-известные-проблемы).
...
- 🚫 Библиотека GitHub-сборок **не может быть главным сервером**: `mesh_master` намертво `False`,
  вкладки «Меш» и «Магазин» скрыты до команды головного сервера; bit-torrent — всегда `direct`.
